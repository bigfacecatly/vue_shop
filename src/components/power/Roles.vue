            ...
            <el-row :class="['bdbottom',i1 === 0 ? 'bdtop' : '','vcenter' ]" v-for="(item1, i1) in scope.row.children" :key="item1.id">
              ...
              <el-col :span="19">
                ...
                <el-row   :class="[ i2=== 0 ? '' : 'bdtop' ,'vcenter']" v-for="(item2, i2) in item1.children" :key="item2.id">
                  ...
                  <el-col :span="18">
                    <el-tag @close="removeRightById(scope.row,item3.id)"   closable v-for="( item3, i3) in item2.children" :key="item3.id" type="warning">{{ item3.authName }}{{i3}}</el-tag>
                  </el-col>
                </el-row>
              </el-col>
            </el-row>
...
<!--            </pre>-->
    ...
    <!--  添加角色对话框-->
    <el-dialog title="添加角色" :visible.sync="addRoleDg" width="60%" @close="rolesDialogClosed">
      <!--      内容主体区-->
      <el-form :model="addFormRole" :rules="addFormRoleRules" ref="addFormRoleRef" label-width="90px">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="addRoleDg = false">取 消</el-button>
        <el-button type="primary" @click="addFormRoleInfo">确 定</el-button>
      </span>
    </el-dialog>
    <!--    打开编辑角色的对话框-->
    <el-dialog title="编辑角色" :visible.sync="showEditRolesDg" width="70%" @close="rolesDgClosed">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="showEditRolesDg = false">取 消</el-button>
        <el-button type="primary" @click="showEditRolesDgInfo">确 定</el-button>
      </span>
    </el-dialog>
    <!--    分配权限的对话框-->
  ...
  methods: {
    ...
    showEditRolesDgInfo () {
      this.$refs.editRolesRef.validate(async valite => {
        ...
        console.log(res)
        if (res.meta.status !== 200) {
          return this.$message.error('角色数据更新失败！')
        }
        // 关闭对话框
