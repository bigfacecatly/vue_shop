...
export default {
  data () {
    ...
    var checkEmail = (rule, value, cb) => {
      // 邮箱验证正则
      const regEmail = /^([a-zA-Z0-9_-])+@([0-9A-Za-z])+(\.[a-zA-Z0-9])+/
    ...
    var checkMobile = (rule, value, cb) => {
      // 手机号正则
      const regMobile = /^(0|86|17952)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/
      if (regMobile.test(value)) {
        return cb()
      }
      cb(new Error('您输入的手机号格式不正确'))
    }
