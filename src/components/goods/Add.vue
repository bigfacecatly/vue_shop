        ...
        <el-tabs :tab-position=" 'left' "  v-model="activeIndex" :before-leave="beforeTabLeave" @tab-click="tabClicked">
          <el-tab-pane label="基本信息" name="0">
            ...
            <el-form-item label="商品分类" prop="goods_cate">
              <el-cascader v-model="addForm.goods_cate" :options="catelist"
          ...
          <el-tab-pane label="商品图片" name="3">
            ...
            <!-- on-preview 指定预览的事件  -->
            <!-- on-remove 移除图片的操作  -->
            <!-- file-list 指定文件列表 这里不需要 删除  -->
            <!-- list-type 指定图片UI显示的效果  -->
            <el-upload :headers="headerObj"
              :action="uploadURL"
              :on-preview="handlePreview"
              :on-remove="handleRemove"
              list-type="picture" :on-success="handleSucces">
  ...
  methods: {
    ...
    async tabClicked () {
      ...
      if (this.activeIndex === '1') {
        ...
        res.data.forEach(item => {
          ...
          // console.log('怎么可以等于000000', item.attr_vals )
        })
    ...
    add () {
      ...
      this.$refs.addFormRef.validate( async valid => {
        ...
        // 执行天际的业务逻辑
        // 深拷贝 loash cloneDeep(obj)
        const form = _.cloneDeep(this.addForm)
        form.goods_cate = form.goods_cate.join(',')
        // 处理动态参数
        // console.log(11111,this.manyTableData)
        ...
        // console.log(1111,form,22222, this.onlyTableData,3333, this.manyTableData)
        // 发起请求添加商品
        // 商品的名称，必须是唯一的
